...
# ==============================================================================
# Tool Execution Logic (Ported from Reeteshrajesh/llm-council)
# ==============================================================================

def _has_finance_signal(query: str) -> bool:
    """Detect if query is about stock prices or finance."""
    q = query.lower()
...
def _has_calc_signal(query: str) -> bool:
    """Detect if query requires mathematical calculation."""
    q = query.lower()
...
def _has_research_signal(query: str) -> bool:
    """Detect if query is about research or reference lookup."""
    q = query.lower()
...
def run_tools_for_query(query: str, limit: int = 3) -> List[Dict[str, str]]:
    ...
    try:
        ...
        wiki_tool = next((t for t in tools if t.name == "wikipedia"), None)
        arxiv_tool = next((t for t in tools if t.name == "arxiv"), None)
        
        # Finance queries - try stock tool
        if _has_finance_signal(query) and stock_tool:
            tickers = _extract_ticker_candidates(query)
            for ticker in tickers[:limit]:
                try:
                    output = stock_tool.func(ticker)
                    if output and "$" in str(output):
                        results.append({"tool": "stock_data", "result": str(output)})
                except Exception:
                    continue
        
        # Calculation queries
        if _has_calc_signal(query) and calc_tool and len(results) < limit:
            try:
                # Extract math expression from query (simple approach)
                import re
                math_expr = re.search(r'[\d\+\-\*\/\(\)\.\s]+', query)
                if math_expr:
                    output = calc_tool.func(math_expr.group().strip())
                    if output and "Error" not in str(output):
                        results.append({"tool": "calculator", "result": str(output)})
            except Exception:
                pass
        
        # Research queries
        if _has_research_signal(query) and len(results) < limit:
            if wiki_tool:
                try:
                    output = wiki_tool.func(query)
                    if output:
                        # Truncate long outputs
                        output_str = str(output)[:500] + "..." if len(str(output)) > 500 else str(output)
                        results.append({"tool": "wikipedia", "result": output_str})
                except Exception:
                    pass
            
            if arxiv_tool and len(results) < limit:
                try:
                    output = arxiv_tool.func(query)
                    if output:
                        output_str = str(output)[:500] + "..." if len(str(output)) > 500 else str(output)
                        results.append({"tool": "arxiv", "result": output_str})
                except Exception:
                    pass
...
def _extract_ticker_candidates(text: str) -> List[str]:
    ...
    stop_words = {
        ...
        "WHAT", "IS", "ARE", "OF", "IN", "ON", "TO", "BY", "GOOD", "SHARE", "SHARES"
    }
    ...
    for tok in tokens:
        ...
        # Only keep 1-5 letter tokens as potential tickers
        if 1 <= len(tok) <= 5 and tok not in seen:
            seen.add(tok)
...
async def stage1_collect_responses(user_query: str, search_context: str = "", request: Any = None) -> Any:
    ...
    try:
        from .documents import get_active_documents_context
    ...
    # Build tool context if tools are enabled and query requires them
    tool_context_block = ""
    try:
        from .tools import get_available_tools
        ...
        if tool_config.get("enable_tools", False):
            tool_outputs = run_tools_for_query(user_query)
            if tool_outputs:
                tool_context_block = "\n\nTool outputs:\n" + "\n".join(
                    f"- {item['tool']}: {item['result']}" for item in tool_outputs
                ) + "\n"
                logger.info(f"Tools executed: {[t['tool'] for t in tool_outputs]}")
    except Exception as e:
        logger.warning(f"Error running tools: {e}")
    ...
    try:
        ...
        # Add context blocks (tools, documents)
        if tool_context_block:
            prompt = tool_context_block + prompt
        if document_context_block:
            prompt = document_context_block + prompt
    except (KeyError, AttributeError, TypeError) as e:
        logger.warning(f"Error formatting Stage 1 prompt: {e}. Using fallback.")
        prompt = f"{document_context_block}{tool_context_block}{search_context_block}Question: {user_query}" if (document_context_block or tool_context_block or search_context_block) else user_query
